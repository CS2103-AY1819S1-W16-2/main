...
image::command_volunteer_add_before.png[add command, 300]
...
|===
.Output of `add`
image::command_volunteer_add_after1.png[add command, 300]
...
|===
.Output of `add`
image::command_volunteer_add_after2.png[add command, 300]
...
image::command_volunteer_edit_before.png[Before edit, 300]

.The targeted volunteer is updated to reflect the new values after entering the edit command
image::command_volunteer_edit_after.png[After edit, 300]
--
...
image::command_volunteer_delete_before.png[Before delete, 300]

.The targeted volunteer is deleted after entering the delete command
image::command_volunteer_delete_after.png[After delete, 300]
--
...
[[command-volunteer-exportvolunteerxml]]
==== Exporting volunteer information as XML: `exportvolunteerxml`

Exports information about a volunteer's information and events participated as an XML file to the user's desktop. This XML file can then ported.

Format: `exportvolunteerxml VOLUNTEER_INDEX`

* Exports the volunteer's information at the specified index
* VOLUNTEER_INDEX must be a positive integer
* If an invalid index is given (ie: a negative number, or one that does not exist), an error message saying invalid input will be displayed
* If the system is unable to generate the file due to internal issues, an error message saying unable to export will be displayed. Please contact us for support if this persists.

Example: 

* `exportvolunteerxml 2` +
Exports the volunteer information at index 2 to the desktop. The XML filename uses the volunteer's name.


[[command-volunteer-exportcsv]]
==== Exporting volunteer information as CSV: `exportcsv`

Exports a volunteer's details as a CSV file. It follows the format of name:address:phone:email:birthday:ID:gender:tags and will have those as the header of the column.

Format: `exportcsv VOLUNTEER_INDEX`
* Exports the volunteer's information at the specified index.
* VOLUNTEER_INDEX must be a positive integer.
* If an invalid index is given (ie: a negative number, or one that does not exist), an error message saying invalid input will be displayed.
* If the system is unable to generate the file due to internal issues, an error message saying unable to export will be displayed. Please contact us for support if this persists.
...
* `exportcsv 2` +
Exports the volunteer information at index 2 to the desktop. The CSV filename uses the volunteer's name.
...
Format: `list` +
Displays all events in the system in the panel. +
The figure below shows the expected panel after executing the `list` command.

.Output of `list`
image::command_event_list.png[list command, 300]
...
--
.Before entering the edit command
image::command_event_edit_before.png[Before edit, 300]

.The targeted event is updated to reflect the new values after entering the edit command
image::command_event_edit_after.png[After edit, 300]
--
...
image::command_event_delete_before.png[Before delete, 300]

.The targeted event is deleted after entering the delete command
image::command_event_delete_after.png[After delete, 300]
--

[[command-event-exportcsv]]
==== Exporting event information as CSV: `exportcsv`

Exports an event's details as a CSV file. It follows the format of name:startDate:startTime:endDate:endTime:Location:Description and will have those as the header of the column.

Format: `exportcsv EVENT_INDEX`
* Exports the event information at the specified index.
...
* `exportcsv 2`
Exports the event information at index 2 to the desktop. The CSV file uses the event's name.


[[command-event-exporteventxml]]
==== Exporting volunteer information as XML: `exporteventxml`

Exports information about an event, detailing the start date/time & end date/time.

Format: `exporteventxml EVENT_INDEX`

* Exports the event's information at the specified index.
* EVENT_INDEX must be a positive integer.
* If an invalid index is given (ie: a negative number, or one that does not exist), an error message saying invalid input will be displayed.
* If the system is unable to generate the file due to internal issues, an error message saying unable to export will be displayed. Please contact us for support if this persists.
...
* `exporteventxml 2` +
Exports the event information at index 2 to the desktop. The XML filename uses the event's name.
